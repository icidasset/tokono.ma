...
Besides all that, there's a few use cases that I consider to a bit more personal, so I generated these tools with the provided Diffuse LLM skill (see [Create page](https://elements.diffuse.sh/latest/create/) on the dashboard) and saved them into my "account":
...
I have some other tools I still want to build, some for data processing (eg. fix metadata). The cool thing is, I can just add or remove these whenever I see fit.
...
- <small>If you'd like to live on the edge, there is the nightly version that does automatically update (even more unstable than alpha versions).</small>
...
Also, special shoutout to @multikatt who has been [donating](https://ko-fi.com/toko) for three years!
